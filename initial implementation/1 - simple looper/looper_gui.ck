//---------------------------------------------------------------------
// name: looper_gui.ck
// desc: ChuGL GUI for multi-track looper.ck control
//       Provides independent controls for each track
//
// Usage:
//    Run alongside looper.ck:
//    chuck looper.ck looper_gui.ck
//---------------------------------------------------------------------

// Setup GWindow
GWindow.windowed(500, 500);
GWindow.center();
GWindow.title("ChucK Multi-Track Looper");
...
// OSC receiver for tempo pulses from looper
OscIn oin;
OscMsg msg;
...
<<< "ChucK Multi-Track Looper GUI Started" >>>;
...
// === MAIN GUI LOOP ===
while(true) {
    if(UI.begin("ChucK Multi-Track Looper")) {

        UI.text("Multi-Track Looper");
        UI.separator();
        UI.spacing();
        ...
        if(beat_on) {
            if(current_beat == 0) {
                UI.pushStyleColor(UI_Color.Button, @(1.0, 0.3, 0.3, 1.0));  // Red for downbeat
            } else {
                UI.pushStyleColor(UI_Color.Button, @(0.3, 1.0, 0.3, 1.0));  // Green
            }
        ...
        for(0 => int track; track < NUM_TRACKS; track++) {
            ...
            if(waiting_to_record[track]) {
                ...
                if(UI.button("Cancel##track" + track)) {
                    clearTrack(track);  // Clear cancels waiting
                }
