//---------------------------------------------------------------------
// name: chuloopa_drums_v3_ableton.ck
...
//       V3 Ableton fork: Replaces SndBuf sample playback with MIDI output
//       via macOS IAC Driver for routing into Ableton Live (or any DAW).
//
// Architecture:
...
//   4. DRUM PLAYBACK: Transcribed drums sent as MIDI notes via IAC Driver
...
fun void setTrackVolume(int track, float vol) {
    // In MIDI mode, volume is controlled by MIDI velocity scaling - no gain node to adjust.
    <<< "Track", track, "Volume:", (Math.max(0.0, Math.min(1.0, vol)) * 100) $ int, "% (MIDI mode: adjust via DAW)" >>>;
}
...
<<< "" >>>;
<<< "=====================================================" >>>;
<<< "      CHULOOPA - AI Drum Variation System (Ableton)" >>>;
<<< "=====================================================" >>>;
...
<<< "" >>>;
<<< "MODE: MIDI DRUMS via IAC Driver (real-time drum feedback in Ableton)" >>>;
<<< "=====================================================" >>>;
...
spork ~ oscListener();  // Listen for OSC messages from Python
...
<<< "" >>>;
<<< "✓ CHULOOPA (Ableton MIDI mode) ready!" >>>;
<<< "" >>>;
<<< "Quick Start:" >>>;
...
<<< "Make sure IAC Driver Bus 1 is enabled in Audio MIDI Setup!" >>>;
<<< "" >>>;
