//---------------------------------------------------------------------
// name: drum_classifier_knn.ck
// desc: Real-time drum classification using ChucK's KNN (K-Nearest Neighbors)
//       Trains KNN from training_samples.csv, then classifies live input
//
// Features:
//   - Loads training data from CSV
//   - Trains KNN classifier in ChucK (no Python needed!)
//   - Real-time onset detection + classification
//   - Audio + visual feedback for classified drums
//
// Usage:
//   chuck src/drum_classifier_knn.ck
...
// KNN parameter
3 => int K_NEIGHBORS;  // Use 3 nearest neighbors
...
// === KNN TRAINING FROM CSV ===

fun int trainKNNFromTXT(string filename) {
    <<< "" >>>;
    <<< "╔═══════════════════════════════════════╗" >>>;
    <<< "║  TRAINING KNN FROM TXT               ║" >>>;
    <<< "╚═══════════════════════════════════════╝" >>>;
    <<< "" >>>;
    ...
    int training_labels[num_samples];      // Labels as int for KNN
    ...
    while(fin.more()) {
        ...
        label_idx => training_labels[sample_idx];
    ...
    // Train KNN
    <<< "Training KNN classifier (K=" + K_NEIGHBORS + ")..." >>>;
...
fun void classifyAndRespond(float flux) {
    ...
    // Predict class probabilities with KNN2
    float probabilities[3];  // One probability per class
    knn.predict(features, K_NEIGHBORS, probabilities);

    // Find class with highest probability
    0 => int predicted_class;
    probabilities[0] => float max_prob;
    for(1 => int i; i < 3; i++) {
...
<<< "" >>>;
<<< "╔═══════════════════════════════════════════════════╗" >>>;
<<< "║  REAL-TIME DRUM CLASSIFIER (ChucK KNN)          ║" >>>;
<<< "╚═══════════════════════════════════════════════════╝" >>>;
<<< "" >>>;

// Train KNN from TXT
if(trainKNNFromTXT("training_samples.txt")) {
    1 => model_trained;
