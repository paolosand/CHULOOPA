...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{assets/chuloopa_system_diagram.png}
  \caption{CHULOOPA system architecture. Beatbox audio flows through onset detection, MFCC-13 KNN classification, and real-time drum playback into a looper, while a background Python process monitors for newly recorded patterns and automatically generates a 5-variant AI variation bank. Audio-driven spice detection selects the best variation at each loop boundary.}
...
\textbf{Feature extraction} for drum classification has been extensively studied. \citet{herrera2003automatic} conducted a comprehensive comparison of feature selection methods for automatic drum sound classification. \citet{sinyor2005beatbox} classified vocal percussion sounds (bass drum, hi-hat, snare) using spectral and temporal features (zero-crossing rate, spectral rolloff, RMS energy, compactness), achieving 95.55\% accuracy with AdaBoost+C4.5 decision trees.

\textbf{Frequency-based discrimination} between kick, snare, and hi-hat is well-established for acoustic instruments. Kick drums exhibit peak energy around 64 Hz \citep{lartillot2007unified}. Snare drums show mid-frequency emphasis (200--500 Hz). Hi-hats concentrate energy in high frequencies ($>$6 kHz) \citep{sillanpaa2000classification}. However, beatbox sounds produced by the human vocal tract do not reproduce these frequency distributions cleanly, motivating features designed for voice rather than acoustic percussion.

\textbf{MFCC for voice-derived percussion:} \citet{martanto2025beatbox} systematically compared Spectral Centroid, Spectral Magnitude, Spectral Contrast, and MFCC (22 coefficients) across KNN, SVM, and AdaBoost classifiers for beatbox classification. MFCC achieved the highest performance across all KNN and non-linear SVM configurations. The authors attribute this to voice-derived beatbox sounds being ``better identified when using a feature more commonly used in identifying the human voice (MFCC)\ldots compared to features more commonly used in identifying percussion sounds.'' This directly validates choosing MFCCs over spectral band energies for amateur beatbox classification.
...
CHULOOPA uses 13 MFCC coefficients---within the standard speech range validated by \citet{martanto2025beatbox}---as its feature representation, capturing timbral shape in a perceptually-motivated space suited to voice-derived percussion.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{assets/chuloopa - active state.png}
  \caption{CHULOOPA ChuGL visualization in the active variation state. The sphere turns blue during variation playback. Spice level text (center) updates in real-time from audio-driven analysis via \texttt{spice\_detector.ck}, color-coded from blue (conservative) through orange (balanced) to red (experimental).}
  \label{fig:ui_active}
\end{figure}
...
When an onset is detected, the system extracts 13 Mel-Frequency Cepstral Coefficients (MFCCs). MFCCs encode timbral shape by applying a mel-scaled filterbank (perceptually weighted to match human hearing) to the power spectrum, then decorrelating via Discrete Cosine Transform. The result is a compact representation of spectral envelope shape rather than energy at specific frequencies.

We chose MFCCs over spectral band energies based on direct experimental evidence from \citet{martanto2025beatbox}, who compared MFCC against Spectral Magnitude, Spectral Contrast, and Spectral Centroid for beatbox classification. MFCC achieved the highest performance across KNN and non-linear SVM models. Critically, the authors explain that voice-derived percussion is better characterized by voice-oriented features (MFCC) than by percussion-oriented spectral features---because the human vocal tract does not produce drum-like frequency content at 64~Hz (kick), 200--500~Hz (snare), or $>$6~kHz (hi-hat). Amateur beatboxers in particular show high intra-user timbral consistency in MFCC space even when their sounds lack the expected frequency-domain properties.

We use 13 coefficients, within the speech-recognition range (12--13) cited by \citet{martanto2025beatbox} as standard for voice feature extraction. With 30 training samples (10 per class), KNN's instance-based learning remains well-conditioned at 13 dimensions: the classifier makes no parametric assumptions and does not overfit, as each class is represented by 10 reference points in MFCC space.
...
During inference, the system computes Euclidean distance in 13-dimensional MFCC space between the current feature vector and all 30 training samples, classifying the onset as the majority class among the 3 nearest neighbors.

\textbf{Fallback Heuristic Classifier:} If the KNN model fails to load, CHULOOPA falls back to a rule-based heuristic classifier using basic spectral energy ratios. While less accurate ($\sim$70\%), this ensures the system remains functional.
...
When a pattern is recorded, the Python engine generates a \textbf{5-variant bank} (var1.txt--var5.txt) in parallel---one thread per reachable slot, all launched simultaneously. This bank enables weighted probabilistic variation selection during performance:
...
\item Spawns one generation thread per reachable slot (1--5, determined by spice ceiling)
\item Each thread independently generates a continuation at \textbf{fixed temperature (0.9)} with a per-slot internal spice value (slot 1: 0.2, slot 2: 0.4 \ldots slot 5: 1.0)
\item Per-slot spice controls \textbf{token count multiplier} (0.85$\times$--1.5$\times$ context length): higher slots generate more tokens, producing denser fills
\item Scales timestamps proportionally to match original loop duration
\item Saves to track\_0\_drums\_var\textit{N}.txt as each thread completes
\item Sends \texttt{/chuloopa/bank\_ready} OSC to ChucK when slot 1 completes---enabling auto-switching before the full bank finishes
\end{enumerate}
...
\textbf{Note:} rhythmic\_creator uses \textbf{fixed temperature (0.9)} for stable generation across all slots. The ``spice level'' (CC 74) controls \emph{which slots are reachable} (see below), not model temperature. For Gemini, spice maps directly to sampling temperature.
...
Users control variation intensity in real-time via MIDI CC 74 (``spice level'', 0.0--1.0). For rhythmic\_creator, spice acts as a \textbf{ceiling} that gates how many variation slots are reachable---determining the size and diversity of the generated bank:

\textbf{For rhythmic\_creator (default):}
\begin{itemize}
\item \textbf{0.0--0.1:} No variations generated
\item \textbf{0.1--0.3:} Slot 1 only (internal spice 0.2, token multiplier $\sim$0.98$\times$)
\item \textbf{0.3--0.5:} Slots 1--2 (up to spice 0.4, multiplier $\sim$1.11$\times$)
\item \textbf{0.5--0.7:} Slots 1--3 (up to spice 0.6, multiplier $\sim$1.24$\times$)
\item \textbf{0.7--0.9:} Slots 1--4 (up to spice 0.8, multiplier $\sim$1.37$\times$)
\item \textbf{0.9--1.0:} All 5 slots (up to spice 1.0, multiplier $\sim$1.50$\times$)
\end{itemize}

Each slot's internal spice controls its \textbf{token count multiplier} (0.85$\times$--1.5$\times$ context length). More tokens means more potential fills above the original pattern density. Lowering spice mid-performance trims the active bank without restarting generation. This design gives performers control over \emph{how much} creative departure is available, rather than a single scalar creativity dial.
...
The current system generates a 5-variant bank per loop with weighted probabilistic selection. Future work includes improving variation density consistency (currently 3--13 hits from a 4-hit input) and developing smarter selection strategies---for example, selecting variations whose density and rhythmic character best match the performer's live input energy.
